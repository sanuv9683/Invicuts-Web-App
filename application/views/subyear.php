	...
	<div class="container">
		...
		<div class="row">
			<div id="content" class="col col-5 col-lg-5">
				...
				<h1 id="sanu">Sub Years Manage</h1>
				<form method="post" action="saveYear">
					...
					<div class="form-group ">
						...
						<input class="form-control" type="text" id="yid" name="yearid">
					</div>
					<div class="form-group ">
						<label>Enter Year
						</label>
...
<script type="application/javascript">
	...
	$("#sub").change(function () {
		...
		$.ajax({
			...
			for(var index in sub){
				var ff=	sub[index];
				 $("#tblMe").append("<tr class='tra'><td>" + ff.yearID + "</td><td>" + ff.yearName + "</td></tr>");
			}
		});
	});
	...
	function maxID() {
		$.ajax({
			method: "GET",
			url: "<?php echo base_url()?>MaxID/getymax",
			async: true,
			dataType: "json"
		}).done(function (response) {
			var aa = (response);
