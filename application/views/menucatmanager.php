	...
	<div class="container" style="min-height: 485px;">
		...
		<div class="row">
			<div id="content" class="col col-5 col-lg-5">
				...
				<h1 id="sanu">Menu Cato Manager</h1>
				<form method="post" action="saveMenuCato">
					...
					<button id="btnCancel" id="btnCancel" type="reset" class="btn  btn-primary">Cancel</button>
				</form>
				<!-- # No more changes -->
			</div>
...
<script type="application/javascript">
	...
	$("#sidd").change(function () {
		...
		$.ajax({
			...
			for (var index in subjects) {
				var subject = subjects[index];
				$("#tblMe").append("<tr class='tra'><td>" + subject.catoID + "</td><td>" + subject.catoType + "</td></tr>");
			}
		});
	});
	...
	function maxID() {
		$.ajax({
			method: "GET",
			url: "<?php echo base_url()?>MaxID/getmcmax",
			async: true,
			dataType: "json"
		}).done(function (response) {
			var aa = (response);
